...
public class StringUtil {
	...
	public static boolean hasLength(String src){
		//有值返回true 没值返回false
		return src!=null && src!="" && !src.isEmpty();
	}
	//方法2：判断源字符串是否有值，空引号(空白字符串)和空格也算没值 (5分)
	public static boolean hasText(String src){
		//有值返回true 没值返回false 空引号(空白字符串)和空格也算没值
		return src!=null && src!="" && !src.isEmpty() && !src.trim().isEmpty();
	}
	...
	public static String randomChineseString(int length){
		...
		for(int i=0;i<length;i++){
			...
			int low,high;
			Random r=new Random();
			high=176+Math.abs(r.nextInt(40));
			r=new Random();
			low=161+Math.abs(r.nextInt(94));
			byte [] b=new byte[2];
